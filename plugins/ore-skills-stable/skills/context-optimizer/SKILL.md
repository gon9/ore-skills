...
- **コンテキスト使用率が約60%に達したとき**（auto-compact の自動発動を待たない — 自動圧縮はタスクの途中で発動し、否定指示・命名規則・過去の決定が失われやすい）
- **タスクの区切り**（PR マージ後、デバッグの原因特定後、調査→実装への切り替え前）
- 設計・方針決定が完了し、実装フェーズに移る前
...
## 制約・禁止事項
- [「〜を使うな」「〜は変更禁止」などの否定指示 — 圧縮で最も失われやすいため必ず明記]
- [命名規則・コーディング規約でセッション中に確立したもの]
...
- **< 200 行** を目標とする（200行を超えるとコンテキスト汚染・性能劣化の報告あり。超える場合はテーマごとに複数ファイルへ分割する）
...
チェックポイント保存後、ユーザーに次の流れを提案する:

1. `/clear` でセッションをリセットする
2. 新しいセッションで保存したチェックポイントを読み込んでから作業を再開する

会話履歴を持ち越すより、ドキュメント経由で文脈を引き継ぐ方が確実で低コスト（毎ターンの履歴再送がなくなり、圧縮による情報欠落も起きない）。
