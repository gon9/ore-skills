...
@patch.dict(os.environ, {"OPENAI_API_KEY": "dummy_key"})
@patch("video_summary.summarizer.OpenAI")
def test_generate_summary_json_success(mock_openai, temp_transcript_json, tmp_path):
    mock_client = MagicMock()
    mock_openai.return_value = mock_client
    
    mock_response = MagicMock()
    mock_response.choices[0].message.content = "要約テキストです。"
...
@patch.dict(os.environ, {"OPENAI_API_KEY": "dummy_key"})
@patch("video_summary.summarizer.OpenAI")
def test_generate_summary_txt_success(mock_openai, temp_transcript_txt, tmp_path):
    mock_client = MagicMock()
    mock_openai.return_value = mock_client
