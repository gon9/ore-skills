...
When a design or decision phase ends and implementation begins, do not carry
the conversation history forward and do not rely on auto-compaction (it fires
mid-task and drops negative instructions, naming conventions, and earlier
decisions). Instead:

1. Persist the decisions to a Markdown doc in the project's `docs/`
   (background, decision, rationale, implementation plan). Keep each doc under
   200 lines; split by theme if larger.
2. Suggest the user `/clear`, then start implementation in a fresh session
   that reads the doc first.

The same applies to subagent handoff packets: a small written doc beats a long
shared history.
