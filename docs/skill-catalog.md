...
| [ocr-local](../skills/ocr-local/SKILL.md) | 実装済 | PSN-69 | python, references | ローカル Tesseract OCR と Playwright 自動スクショでスクリーンショットを Markdown に文字起こしする。外部 API 不使用(社外秘要件)。Use when the user needs to OCR screenshots, capture web pages, or transcribe slide decks locally. |
