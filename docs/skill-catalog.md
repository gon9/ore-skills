...
| [efficient-fable](../skills/efficient-fable/SKILL.md) | 実装済 | 既存・手動抽出 | assets | Use when running Claude Fable on codebase-heavy or token-heavy work and the user wants Fable to orchestrate research, coding, and testing while cheaper subagents do bounded heavy lifting. |
...
| [ocr-local](../skills/ocr-local/SKILL.md) | 実装済 | PSN-69 | python, references | ローカル Tesseract OCR でスクリーンショット画像群を Markdown に文字起こしする。Playwright による自動スクショ取得も可能。社外秘ドキュメントを外部 API に出さずに処理したいときに使う。 |
...
| [video-summary](../skills/video-summary/SKILL.md) | 実装済 | 既存・手動抽出 | scripts | Extracts audio from video files, transcribes speech using Whisper API, and summarizes the content. Use when the user wants to summarize a video, transcribe audio, or extract key points from video content. |
