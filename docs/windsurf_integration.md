...
`scripts/setup-windsurf.sh` を使って自動セットアップできます。

#### 動作の概要

スクリプトは実行場所に応じて自動的に動作を切り替えます:

- **ore-skills リポジトリ内で実行** → Global Skills として自動統合（循環防止のため Workspace は不可）
- **外部プロジェクトのルートで実行** → Workspace / Global を選択可能

#### 使い方

```bash
# パターン A: Global Skills（ore-skills を直接クローンした場合）
~/ore-skills/scripts/setup-windsurf.sh

# パターン B: Workspace Skills（ore-skills をサブモジュールとして追加した場合）
cd /path/to/your-project
./.ore-skills/scripts/setup-windsurf.sh
# → 「1) Workspace Skills」を選択
```

> **⚠️ 注意:** ore-skills リポジトリ内で Workspace Skills を選択すると、ore-skills 自身の中に
> `.windsurf/skills/` が作成され、自分自身へのシンボリックリンクが生成されてしまいます。
> これを防ぐため、スクリプトは ore-skills 内での実行を検出し、自動的に Global Skills を選択します。
