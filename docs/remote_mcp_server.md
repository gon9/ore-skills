...
> 実験的な過去設計メモです。現在の実装はstdioのみで、リモートtransportは提供していません。実装を再開する場合は、その時点のMCP仕様に沿ってtransport、認証、認可を再設計してください。

## 概要

現在の `ore-skills-server` は **stdio (標準入出力) ベース** のローカル専用MCPサーバーです。
このドキュメントでは、EC2などでホストする案として検討した **SSE (Server-Sent Events) ベース** の旧設計を記録します。
